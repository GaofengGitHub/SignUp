				...
				if(dom.collectState.classList.contains('collect-state-active')) {
					...
					c.dataUtil.cancelCollect(pageId, "1", function(res) {
						...
						if(res.SystemCode == 1) {
							...
							$.fire(plus.webview.currentWebview().opener(), 'addCollect', {});
							//.collect-state  .collect-state-active .u-func-favor-active  .u-func-favor
							//							collectState.style.color = '#000000';
							//							collectState.innerHTML = "未收藏";
							//							collectStar.style.color = '#000000';
						} else {
							$.toast('取消失败');
						}
					...
					c.dataUtil.collectActivity(pageId, '1', function(res) {
						...
						if(res.SystemCode == 1) {
							...
							$.fire(plus.webview.currentWebview().opener(), 'addCollect', {});
							//							dom.collectState.classList.add('collect-state-active');
							//							collectState.style.color = '#EB3A3A';
							//							collectState.innerHTML = "已收藏";
							//							collectStar.style.color = '#EB3A3A';
...
//				}
//
//				if(res.data.isFinish == 0 && res.data.isApply == 0 && res.data.isCeil == 0) {
//					dom.signUp.style.display = "block";
//				}
